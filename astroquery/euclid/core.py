...
class EuclidClass(object):
    ...
    """
    Proxy class to default TapPlus object (pointing to the archive)
    """

    def __init__(self, tap_plus_handler=None):
        if tap_plus_handler is None:
            self.__taphandler = TapPlus(url="http://eas.esac.esa.int/tap-dev/tap")
        else:
    ...
    def __cone_search(self, coordinate, radius, async_job=False,
        ...
        query = "SELECT DISTANCE(POINT('ICRS',"+str(MAIN_TABLE_RA)+","\
            ...
            POINT('ICRS',"+str(ra)+","+str(dec)+")) AS dist, * \
            FROM "+str(MAIN_TABLE)+" WHERE CONTAINS(\
            POINT('ICRS',"+str(MAIN_TABLE_RA)+","+str(MAIN_TABLE_DEC)+"),\
    ...
    def login(self, user=None, password=None, credentials_file=None,
        ...
        verbose : bool, optional, default 'False'
            flag to display information about the process
        """
        return self.__taphandler.login(user=user,
                                    password=password,
    ...
    def login_gui(self, verbose=False):
        ...
        verbose : bool, optional, default 'False'
            flag to display information about the process
        """
        return self.__taphandler.login_gui(verbose)
